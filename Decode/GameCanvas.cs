...
public class GameCanvas : IActionListener
{
	...
	public void initGame()
	{
		...
		Command.btn0left = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btn0left") : loadImage("/mainImage/btn0left.png"));
		Command.btn0mid = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btn0mid") : loadImage("/mainImage/btn0mid.png"));
		Command.btn0right = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btn0right") : loadImage("/mainImage/btn0right.png"));
		Command.btn1left = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btn1left") : loadImage("/mainImage/btn1left.png"));
		Command.btn1mid = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btn1mid") : loadImage("/mainImage/btn1mid.png"));
		Command.btn1right = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btn1right") : loadImage("/mainImage/btn1right.png"));
	...
	public void update()
	{
		...
		try
		{
			...
			{
				if (!FunctionLogin.CheckLogin && gameTick % 10 == 0)
				{
					FunctionLogin.GetData();
					Thread thread = new Thread(FunctionLogin.gI().Login);
					thread.IsBackground = true;
					thread.Start();
					FunctionLogin.CheckLogin = true;
				}
				...
				if (!isLoading)
				{
					...
					FunctionLogin.gI().AutoLogin();
					FunctionLogin.gI().FixBlackScreen();
				}
				debug("F", 0);
	...
	public static void paintBGGameScr(mGraphics g)
	{
		...
		if (FunctionGraphic.enablePaintImage_Wallpaper)
		{
			if (FunctionGraphic.Wallpaper != null)
			{
				g.drawImage(FunctionGraphic.Wallpaper, w / 2, h / 2, 3);
				return;
			}
		}
		else if (FunctionGraphic.enablePaintColor_Wallpaper)
		{
			g.translate(-g.getTranslateX(), -g.getTranslateY());
			g.setColor(FunctionGraphic.ColorRGB);
			g.fillRect(0, 0, w, h);
			return;
		}
		if (!isLoadBGok)
	...
	public void keyPressedz(int keyCode)
	{
		...
		if (menu.showMenu)
		{
			...
			{
			case 49:
				menu.menuSelectedItem = keyCode - 49;
				gI().keyPressedz(-5);
				break;
			case 50:
				menu.menuSelectedItem = keyCode - 49;
	...
	public void paint(mGraphics gx)
	{
		try
		{
			...
			else if (!Char.isLoadingMap || !LoginScr.isContinueToLogin || !ServerListScreen.waitToLogin || !ServerListScreen.isWait)
			{
				timeout = 0;
			}
			debug("PE", 1);
	...
	public static void startOKDlg(string info)
	{
		if (!info.ToLower().StartsWith("không thể đặt") && !info.ToLower().Contains("không thể đổi khu vực trong map này") && !info.ToLower().Contains("can not change zone in this map"))
		{
			...
			else if (info.ToLower().StartsWith("vui lòng đợi"))
			{
