...
public class mFont
{
	...
	public void drawString(mGraphics g, string st, int x, int y, int align)
	{
		if (mGraphics.zoomLevel == 1)
		{
			...
			{
				int num3 = strFont.IndexOf(st[i] + string.Empty);
				if (num3 == -1)
				{
					num3 = 0;
				}
				if (num3 > -1)
				{
					int x2 = fImages[num3][0];
					int num4 = fImages[num3][1];
					int w = fImages[num3][2];
					int num5 = fImages[num3][3];
					if (num4 + num5 > imgFont.texture.height)
					{
						num4 -= imgFont.texture.height;
						x2 = imgFont.texture.width / 2;
					}
					g.drawRegion(imgFont, x2, num4, w, num5, 0, num2, y, 20);
				}
				num2 += fImages[num3][2] + space;
			}
		}
		else
		{
