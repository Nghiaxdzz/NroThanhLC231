...
public class ChatPopup : Effect2, IActionListener
{
	...
	public static void addChatPopupMultiLine(string chat, int howLong, Npc c)
	{
		if (chat.ToLower().StartsWith("chưa thể chuyển khu vực lúc này vui lòng chờ"))
		{
			...
			GameScr.info1.addInfo("Chờ " + num + " giây nữa", 0);
		}
		else
		{
			if (chat.ToLower().StartsWith("không thể chuyển khu vực"))
			{
				return;
			}
			if (chat.ToLower().StartsWith("không thể đến khu vực này"))
			{
				FunctionMap.ZoneRequest = -1;
				return;
			}
			if (chat.ToLower().StartsWith("please wait"))
			{
				int num2 = int.Parse(chat.Split(' ')[10].Split('.')[1]);
				GameScr.info1.addInfo("Chờ " + num2 + " giây nữa", 0);
				return;
			}
