...
public class Panel : IActionListener, IChatable
{
	...
	public Panel()
	{
		...
		cmdClose.img = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("X") : GameCanvas.loadImage("/mainImage/myTexture2dbtX.png"));
	...
	public void setTabPetInventory()
	{
		ITEM_HEIGHT = 24;
		...
		if (cmtoX >= wScroll - 10 && cmx >= wScroll - 10 && position == 0)
		{
			...
			if (isChangeZone)
			{
				...
				if (Char.myCharz().cHP > 0 && Char.myCharz().statusMe != 14)
				{
					...
					{
						FunctionXmap.SelectMapTrans(selected);
					}
				}
			}
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (item != null)
			{
				...
				if (item.itemOption != null)
				{
					for (int j = 0; j < item.itemOption.Length; j++)
					{
						if (TileMap.mapName.ToLower().Contains("thần điện"))
						{
							if (item.template.type != 33)
							{
								text = ((!item.itemOption[j].getOptionString().StartsWith("HSD")) ? " [Vĩnh viễn]" : (" [" + item.itemOption[j].getOptionString().Split(' ')[1] + " ngày]"));
							}
						}
						else
						{
				...
				mFont2.drawString(g, item.template.name + text2 + text, num2 + 5, num3 + 1, 0);
				...
				if (item.itemOption != null && item.itemOption.Length >= 1)
				{
					...
					{
						text3 += item.itemOption[0].getOptionString();
					}
					mFont mFont3 = mFont.tahoma_7_blue;
					if (item.compare < 0 && item.template.type != 5)
					{
					...
					{
						mFont3.drawString(g, text3, num2 + 5, num3 + 11, 0);
					}
				}
				if (item.buySpec > 0)
				{
		...
		for (int i = 0; i < arrItemBody.Length + arrPetSkill.Length; i++)
		{
			...
			if (item != null)
			{
				...
				if (item.itemOption != null)
				{
					for (int k = 0; k < item.itemOption.Length; k++)
					{
						if (!item.itemOption[k].getOptionString().StartsWith("HSD") && item.template.id == 860)
						{
							item.PaintColor(g, xScroll + 18, yScroll + i * ITEM_HEIGHT + 12, 7);
						}
						if (item.itemOption[k].optionTemplate.id == 72)
						{
							item.PaintColor(g, xScroll + 18, yScroll + i * ITEM_HEIGHT + 12, item.itemOption[k].param);
							text = " [+" + item.itemOption[k].param + "]";
						}
						if (item.itemOption[k].optionTemplate.id == 41)
						{
					...
					if (item.itemOption.Length > 1)
					{
						for (int l = 1; l < item.itemOption.Length; l++)
						{
							...
							if ((item.template.type >= 0 && item.template.type <= 4) || item.template.type == 32)
							{
								try
								{
									string[] array = item.itemOption[l].getOptionString().Split(' ');
									bool flag2 = array.Length == 6 && array[0] + array[2] + array[3] + array[4] + array[5] == "BiếntấncôngthànhHP";
									bool flag3 = array.Length == 6 && array[0] + array[2] + array[3] + array[4] + array[5] == "BiếntấncôngthànhKI";
									if (flag2)
									{
										text3 = text3 + " " + array[1] + " HM";
									}
									if (flag3)
									{
										text3 = text3 + " " + array[1] + " HK";
									}
									bool flag4 = item.itemOption[l].getOptionString().Split('+')[0] == "HP" && item.itemOption[l].getOptionString().Split('%')[1] == string.Empty;
									bool flag5 = array.Length == 2 && array[0] == "KI" && item.itemOption[l].getOptionString().Split('+')[0] == "KI ";
									bool flag6 = item.itemOption[l].getOptionString().Split('+')[0] == "Sức đánh";
									bool flag7 = array.Length == 2 && array[0] == "Giáp";
									bool flag8 = array.Length == 2 && array[1] == "TN,SM";
									bool flag9 = item.itemOption[l].getOptionString().Contains("/30s") && item.itemOption[l].getOptionString().Split('+')[0] == "HP";
									bool flag10 = item.itemOption[l].getOptionString().Contains("/30s") && item.itemOption[l].getOptionString().Split('+')[0] == "KI";
									bool flag11 = array.Length == 4 && array[0] + array[2] + array[3] == "Phảnsátthương";
									bool flag12 = array.Length == 3 && array[1] + array[2] == "Néđòn";
									if (flag9)
									{
										text3 = text3 + " " + item.itemOption[l].getOptionString();
									}
									if (flag10)
									{
										text3 = text3 + " " + item.itemOption[l].getOptionString();
									...
									if (flag11)
									{
										text3 = text3 + " " + array[1] + " PST";
									}
									if (flag12)
									{
										text3 = text3 + " " + array[0] + " Né đòn";
									}
									if (flag4)
									{
										text3 = text3 + " " + item.itemOption[l].getOptionString().Split('+')[1] + " HP";
									}
									if (flag5)
									{
										text3 = text3 + " " + item.itemOption[l].getOptionString().Split('+')[1] + " KI";
									}
									if (flag6)
									{
										text3 = text3 + " " + item.itemOption[l].getOptionString().Split('+')[1] + " SĐ";
									}
									if (flag7)
									{
										text3 = text3 + " " + array[1] + " Giáp";
									}
									if (flag8)
									{
										text3 = text3 + " " + item.itemOption[l].getOptionString();
							...
							else if (item.template.type > 4 && item.template.type != 32)
							{
								text2 = text2 + "," + item.itemOption[l].getOptionString();
							}
						}
				...
				mFont.tahoma_7_blue.drawString(g, text3, 170, yScroll + i * ITEM_HEIGHT + 11, mFont.RIGHT);
				if (item.itemOption != null)
				{
					for (int m = 0; m < item.itemOption.Length; m++)
					{
					...
					for (int num10 = 0; num10 < item.itemOption.Length; num10++)
					{
						...
						if (item.itemOption[num10].param > 0)
						{
							...
							{
								text4 = item.itemOption[num10].param.ToString();
							}
						...
						{
							mFont.tahoma_7b_yellowSmall.drawString(g, text4, 160, yScroll + i * ITEM_HEIGHT, 0, mFont.tahoma_7b_dark);
							g.drawImage(imgStar, 165, yScroll + i * ITEM_HEIGHT + 1);
						}
					}
				}
				if (item.quantity > 1)
	...
	private void paintBox(mGraphics g)
	{
		...
		try
		{
			...
			for (int i = 0; i < currentListLength; i++)
			{
				...
				StringHandle.paint(mFont.tahoma_7_white, g, (i + 1).ToString(), xScroll + 2, yScroll + i * ITEM_HEIGHT - 2, 0, mFont.tahoma_7, "border", mGraphics.zoomLevel);
				if (item == null)
				{
					continue;
				}
					...
					if (item.itemOption.Length > 1)
					{
						for (int l = 1; l < item.itemOption.Length; l++)
						{
							...
							if ((item.template.type >= 0 && item.template.type <= 4) || item.template.type == 32)
							{
								...
								{
									string[] array = item.itemOption[l].getOptionString().Split(' ');
									bool flag = array.Length == 6 && array[0] + array[2] + array[3] + array[4] + array[5] == "BiếntấncôngthànhHP";
									bool flag2 = array.Length == 6 && array[0] + array[2] + array[3] + array[4] + array[5] == "BiếntấncôngthànhKI";
									if (flag)
									{
										text3 = text3 + " " + array[1] + " HM";
									}
									...
									bool flag3 = item.itemOption[l].getOptionString().Split('+')[0] == "HP" && item.itemOption[l].getOptionString().Split('%')[1] == string.Empty;
									bool flag4 = array.Length == 2 && array[0] == "KI" && item.itemOption[l].getOptionString().Split('+')[0] == "KI ";
									bool flag5 = item.itemOption[l].getOptionString().Split('+')[0] == "Sức đánh";
									bool flag6 = array.Length == 2 && array[0] == "Giáp";
									bool flag7 = array.Length == 2 && array[1] == "TN,SM";
									bool flag8 = item.itemOption[l].getOptionString().Contains("/30s") && item.itemOption[l].getOptionString().Split('+')[0] == "HP";
									bool flag9 = item.itemOption[l].getOptionString().Contains("/30s") && item.itemOption[l].getOptionString().Split('+')[0] == "KI";
									bool flag10 = array.Length == 4 && array[0] + array[2] + array[3] == "Phảnsátthương";
									bool flag11 = array.Length == 3 && array[1] + array[2] == "Néđòn";
									if (flag8)
									{
										text3 = text3 + " " + item.itemOption[l].getOptionString();
									}
				...
				mFont.tahoma_7_blue.drawString(g, text3, xScroll + wScroll - 2, yScroll + i * ITEM_HEIGHT + 11, mFont.RIGHT);
				if (item.itemOption != null)
				{
					for (int m = 0; m < item.itemOption.Length; m++)
					{
						paintOptItem(g, item.itemOption[m].optionTemplate.id, item.itemOption[m].param, num4, num5, num6, num7);
					}
					for (int n = 0; n < item.itemOption.Length; n++)
					{
						paintOptSlotItem(g, item.itemOption[n].optionTemplate.id, item.itemOption[n].param, num4, num5, num6, num7);
					}
					for (int num8 = 0; num8 < item.itemOption.Length; num8++)
					{
						string text4 = string.Empty;
						if (item.itemOption[num8].param > 0)
						{
							if (item.itemOption[num8].optionTemplate.id == 102)
							{
								text4 = item.itemOption[num8].param.ToString();
							}
							else if (item.itemOption[num8].optionTemplate.id == 107)
							{
								text4 = item.itemOption[num8].param.ToString();
							}
						}
						if (text4 != string.Empty)
						{
							mFont.tahoma_7b_yellowSmall.drawString(g, text4, xScroll + wScroll - 17, yScroll + i * ITEM_HEIGHT, 0, mFont.tahoma_7b_dark);
							g.drawImage(imgStar, xScroll + wScroll - 11, yScroll + i * ITEM_HEIGHT + 1);
						}
					}
				}
				if (item.quantity > 1)
				{
					mFont.tahoma_7_yellow.drawString(g, string.Empty + item.quantity, num4 + num6, num5 + num7 - mFont.tahoma_7_yellow.getHeight(), 1);
				}
			}
		}
		catch (Exception)
	...
	private void paintInventory(mGraphics g)
	{
		...
		try
		{
			...
			for (int i = 0; i < currentListLength; i++)
			{
				...
				g.setColor((i == selected) ? 16383818 : ((i >= arrItemBody.Length) ? 15723751 : 15196114));
				g.fillRect(num, num2, num3, h);
				g.setColor((i == selected) ? 9541120 : ((i >= arrItemBody.Length) ? 11837316 : 9993045));
				Item item = ((i >= arrItemBody.Length) ? arrItemBag[i - arrItemBody.Length] : arrItemBody[i]);
				if (item != null)
				{
					for (int j = 0; j < item.itemOption.Length; j++)
					{
						if (item.itemOption[j].optionTemplate.id == 72 && item.itemOption[j].param > 0)
						{
							byte id = (byte)GetColor_Item_Upgrade(item.itemOption[j].param);
							int color_ItemBg = GetColor_ItemBg(id);
							if (color_ItemBg != -1)
							{
								g.setColor((i != selected) ? GetColor_ItemBg(id) : GetColor_ItemBg(id));
							}
						}
					}
				}
				...
				if (i > num8)
				{
					StringHandle.drawStringBd(mFont.tahoma_7_white, g, (i - num8).ToString(), xScroll + 2, yScroll + i * ITEM_HEIGHT - 2, 0, mFont.tahoma_7);
				}
				if (item == null)
				{
					continue;
				...
				if (item.itemOption != null)
				{
					for (int k = 0; k < item.itemOption.Length; k++)
					{
						...
						{
							item.PaintColor(g, xScroll + 18, yScroll + i * ITEM_HEIGHT + 12, 5);
						}
						if (item.itemOption[k].optionTemplate.id == 41)
						{
							if (item.itemOption[k].param == 1)
				...
				string text2 = string.Empty;
				string text3 = string.Empty;
				foreach (FunctionItem.Equipment1 item2 in FunctionItem.ListEquipment1)
				{
					if (item.template.type == item2.type && item.info == item2.info)
					{
						text2 = " - Set1";
					}
				}
				foreach (FunctionItem.Equipment2 item3 in FunctionItem.ListEquipment2)
				{
					if (item.template.type == item3.type && item.info == item3.info)
					{
						text3 = " - Set2";
					}
				}
				...
				{
					StringHandle.paint(mFont.tahoma_7b_dark, g, item.template.name + text + text2, num + 5, num2 + 1, 0, mFont.tahoma_7b_white, "border", mGraphics.zoomLevel);
				}
				else if (text3 != string.Empty)
				{
					StringHandle.paint(mFont.tahoma_7b_dark, g, item.template.name + text + text3, num + 5, num2 + 1, 0, mFont.tahoma_7b_white, "border", mGraphics.zoomLevel);
				}
				else
				{
					mFont2.drawString(g, item.template.name + text, num + 5, num2 + 1, 0);
				...
				if (item.itemOption != null)
				{
					if (item.itemOption.Length != 0 && item.itemOption[0] != null && item.itemOption[0].optionTemplate.id != 102 && item.itemOption[0].optionTemplate.id != 107)
					{
						text4 += item.itemOption[0].getOptionString();
					}
					mFont mFont3 = mFont.tahoma_7_blue;
					if (item.compare < 0 && item.template.type != 5)
					{
					...
					if (item.itemOption.Length > 1)
					{
						for (int l = 1; l < 2; l++)
						{
							if (item.itemOption[l] != null && item.itemOption[l].optionTemplate.id != 102 && item.itemOption[l].optionTemplate.id != 107 && item.itemOption[l].getOptionString().StartsWith("Set"))
							{
								text4 = text4 + "," + item.itemOption[l].getOptionString();
							}
						}
						for (int m = 1; m < item.itemOption.Length; m++)
						{
							if (item.itemOption[m] == null || item.itemOption[m].optionTemplate.id == 102 || item.itemOption[m].optionTemplate.id == 107 || item.itemOption[1].getOptionString().StartsWith("Set"))
							{
								continue;
							}
							if ((item.template.type >= 0 && item.template.type <= 4) || item.template.type == 32)
							{
								try
								{
									string[] array = item.itemOption[m].getOptionString().Split(' ');
									bool flag = array.Length == 6 && array[0] + array[2] + array[3] + array[4] + array[5] == "BiếntấncôngthànhHP";
									bool flag2 = array.Length == 6 && array[0] + array[2] + array[3] + array[4] + array[5] == "BiếntấncôngthànhKI";
									if (flag)
									{
										text5 = text5 + " " + array[1] + " HM";
									}
									if (flag2)
									{
										text5 = text5 + " " + array[1] + " HK";
									}
									bool flag3 = item.itemOption[m].getOptionString().Split('+')[0] == "HP" && item.itemOption[m].getOptionString().Split('%')[1] == string.Empty;
									bool flag4 = array.Length == 2 && array[0] == "KI" && item.itemOption[m].getOptionString().Split('+')[0] == "KI ";
									bool flag5 = item.itemOption[m].getOptionString().Split('+')[0] == "Sức đánh";
									bool flag6 = array.Length == 2 && array[0] == "Giáp";
									bool flag7 = array.Length == 2 && array[1] == "TN,SM";
									bool flag8 = item.itemOption[m].getOptionString().Contains("/30s") && item.itemOption[m].getOptionString().Split('+')[0] == "HP";
									bool flag9 = item.itemOption[m].getOptionString().Contains("/30s") && item.itemOption[m].getOptionString().Split('+')[0] == "KI";
									bool flag10 = array.Length == 4 && array[0] + array[2] + array[3] == "Phảnsátthương";
									bool flag11 = array.Length == 3 && array[1] + array[2] == "Néđòn";
									if (flag8)
									{
										text5 = text5 + " " + item.itemOption[m].getOptionString();
									}
									if (flag9)
									{
										text5 = text5 + " " + item.itemOption[m].getOptionString();
									...
									if (flag10)
									{
										text5 = text5 + " " + array[1] + " PST";
									}
									if (flag11)
									{
										text5 = text5 + " " + array[0] + " Né đòn";
									}
									if (flag3)
									{
										text5 = text5 + " " + item.itemOption[m].getOptionString().Split('+')[1] + " HP";
									}
									if (flag4)
									{
										text5 = text5 + " " + item.itemOption[m].getOptionString().Split('+')[1] + " KI";
									}
									if (flag5)
									{
										text5 = text5 + " " + item.itemOption[m].getOptionString().Split('+')[1] + " SĐ";
									}
									if (flag6)
									{
										text5 = text5 + " " + array[1] + " Giáp";
									}
									if (flag7)
									{
										text5 = text5 + " " + item.itemOption[m].getOptionString();
							...
							{
								text4 = text4 + "," + item.itemOption[m].getOptionString();
							}
						}
					}
					mFont3.drawString(g, text4, num + 5, num2 + 11, mFont.LEFT);
				}
				SmallImage.drawSmallImage(g, item.template.iconID, num4 + num6 / 2, num5 + num7 / 2, 0, 3);
				mFont.tahoma_7_blue.drawString(g, text5, xScroll + wScroll - 2, yScroll + i * ITEM_HEIGHT + 11, mFont.RIGHT);
				if (item.itemOption != null)
				{
					for (int n = 0; n < item.itemOption.Length; n++)
					{
						paintOptItem(g, item.itemOption[n].optionTemplate.id, item.itemOption[n].param, num4, num5, num6, num7);
					}
					for (int num9 = 0; num9 < item.itemOption.Length; num9++)
					{
						paintOptSlotItem(g, item.itemOption[num9].optionTemplate.id, item.itemOption[num9].param, num4, num5, num6, num7);
					}
					for (int num10 = 0; num10 < item.itemOption.Length; num10++)
					{
						...
						if (item.itemOption[num10].param > 0)
						{
							if (item.itemOption[num10].optionTemplate.id == 102)
							{
								text6 = item.itemOption[num10].param.ToString();
							}
							else if (item.itemOption[num10].optionTemplate.id == 107)
							{
								text6 = item.itemOption[num10].param.ToString();
						...
						{
							mFont.tahoma_7b_yellowSmall.drawString(g, text6, xScroll + wScroll - 17, yScroll + i * ITEM_HEIGHT, 0, mFont.tahoma_7b_dark);
							g.drawImage(imgStar, xScroll + wScroll - 11, yScroll + i * ITEM_HEIGHT + 1);
						}
					}
				}
	...
	private void paintTab(mGraphics g)
	{
		if (type == 23 || type == 24)
		{
			for (int i = 0; i < FunctionMenu.MenuOption.Length; i++)
			{
				...
				if (!FunctionMenu.MenuOption[i][1].Equals(string.Empty))
				{
					mFont2.drawString(g, FunctionMenu.MenuOption[i][0], startTabPos + i * TAB_W + TAB_W / 2, 53, mFont.CENTER);
					mFont2.drawString(g, FunctionMenu.MenuOption[i][1], startTabPos + i * TAB_W + TAB_W / 2, 64, mFont.CENTER);
				}
				else
				{
					mFont2.drawString(g, FunctionMenu.MenuOption[i][0], startTabPos + i * TAB_W + TAB_W / 2, 59, mFont.CENTER);
				}
				if (type == 0 && FunctionMenu.MenuOption.Length == 5 && GameScr.isNewClanMessage && GameCanvas.gameTick % 4 == 0)
				{
	...
	private void paintCharInfo(mGraphics g, Char c)
	{
		...
		{
			mFont.tahoma_7_yellow.drawString(g, "ID: " + ((!c.me) ? c.charID : c.charID), X + 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
		}
		g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
		if (c.cPower > 0)
		{
	...
	{
		mFont.tahoma_7_yellow.drawString(g, "HP: " + NinjaUtil.getMoneys(Char.myPetz().cHP) + "/" + NinjaUtil.getMoneys(Char.myPetz().cHPFull), X + 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
		mFont.tahoma_7_yellow.drawString(g, "KI: " + NinjaUtil.getMoneys(Char.myPetz().cMP) + "/" + NinjaUtil.getMoneys(Char.myPetz().cMPFull), X + 60, 16, mFont.LEFT, mFont.tahoma_7b_dark);
			...
			if (currItem != null)
			{
				...
				else if (currItem.buyCoin == 0 && currItem.buyGold == 0)
				{
					...
					else
					{
						...
						myVector.addElement(new Command("Mua Nhiều", FunctionItem.gI(), 3, new FunctionItem.ItemAuto(currItem.template.id, -1, bool_0: true, bool_1: false)));
					}
				}
				else if (typeShop == 8)
				{
				...
				else if (typeShop != 2)
				{
					if (currItem.buyCoin > 0)
					{
						...
						myVector.addElement(new Command("Mua Nhiều\n" + Res.formatNumber2(currItem.buyCoin) + " Vàng/1", FunctionItem.gI(), 3, new FunctionItem.ItemAuto(currItem.template.id, -1, bool_0: true, bool_1: false)));
					}
					if (currItem.buyGold > 0)
					{
						myVector.addElement(new Command(mResources.buy_with + "\n" + Res.formatNumber2(currItem.buyGold) + "\n" + mResources.LUONG, this, 3001, currItem));
						myVector.addElement(new Command("Mua Nhiều\n" + Res.formatNumber2(currItem.buyGold) + " Ngọc/1", FunctionItem.gI(), 3, new FunctionItem.ItemAuto(currItem.template.id, -1, bool_0: false, bool_1: false)));
					}
				}
				else
				{
			...
			if (selected >= arrItemBody.Length)
			{
				...
				if (item != null)
				{
					...
					else if (item.isTypeBody())
					{
						...
						if (position != 1)
						{
							myVector.addElement(new Command("Thêm vào set 1", this, 3142, currItem));
							myVector.addElement(new Command("Thêm vào set 2", this, 3143, currItem));
						}
					}
					else
					{
						myVector.addElement(new Command(mResources.USE, this, 2001, currItem));
						if (position != 1 && item.template.type == 29)
						{
							myVector.addElement(new Command("Thêm vào ds Item", this, 3144, currItem));
						}
					}
				}
			}
	...
	{
		currentListLength = FunctionMenu.MenuOption.Length;
		ITEM_HEIGHT = 24;
		selected = (GameCanvas.isTouch ? (-1) : 0);
		cmyLim = currentListLength * ITEM_HEIGHT - hScroll;
		if (cmyLim < 0)
	...
	private void doFireEnemy()
	{
		if (selected >= 0 && vEnemy.size() != 0)
		{
			...
			myVector.addElement(new Command(mResources.make_friend, this, 8003, (InfoItem)vEnemy.elementAt(currInfoItem)));
			GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
	...
	private void doFireFriend()
	{
		if (selected >= 0 && vFriend.size() != 0)
		{
			...
			myVector.addElement(new Command("Thêm vào đối tượng dịch chuyển", this, 8008, (InfoItem)vFriend.elementAt(currInfoItem)));
			GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
			...
			else if (isViewMember)
			{
				...
				if (currMem != null)
				{
					...
					else if (myMember != null)
					{
						...
						myVector4.addElement(new Command(mResources.den, this, 8005, currMem));
						myVector4.addElement(new Command("Chọn đối tượng cho đậu", this, 8006, currMem));
						myVector4.addElement(new Command(mResources.make_friend, this, 8007, currMem));
					}
	...
	public void perform(int idAction, object p)
	{
		...
		if (idAction == 8004)
		{
			...
			FunctionChar.Teleport(infoItem3.charInfo.charID);
		}
		if (idAction == 8005)
		{
			Member member = (Member)p;
			FunctionChar.Teleport(member.ID);
		}
		if (idAction == 8006)
		{
			Member member2 = (Member)p;
			if (FunctionPet.ListMembers.Contains(member2.name))
			{
				FunctionPet.ListMembers.Remove(member2.name);
				GameScr.info1.addInfo("[ThanhLc] Xóa: " + member2.name + " khỏi d/s nhận đậu", 0);
			}
			else
			{
				FunctionPet.ListMembers.Add(member2.name);
				GameScr.info1.addInfo("[ThanhLc] Thêm: " + member2.name + " vào d/s nhận đậu", 0);
			}
		}
		if (idAction == 8007)
		{
			Member member3 = (Member)p;
			Service.gI().friend(1, member3.ID);
			if (type == 8)
			{
			}
		}
		if (idAction == 8008)
		{
			...
			FunctionChar.enableAutoTeleport = !FunctionChar.enableAutoTeleport;
			FunctionChar.CharIDTeleport = infoItem4.charInfo.charID;
			GameScr.info1.addInfo("Auto giảm sức mạnh: " + StringHandle.Status(FunctionChar.enableAutoTeleport), 0);
		}
		if (idAction == 3142)
		{
			FunctionItem.AddEquipmentstoList(currItem, 1);
		}
		if (idAction == 3143)
		{
			FunctionItem.AddEquipmentstoList(currItem, 2);
		}
		if (idAction == 3144)
		{
			FunctionItem.AddItemstoList(currItem);
		}
		if (idAction == 8001)
		{
			...
			chatTField.tfChat.name = mResources.chat_with + " " + infoItem5.charInfo.cName;
			chatTField.to = string.Empty;
			chatTField.isShow = true;
			chatTField.tfChat.isFocus = true;
			chatTField.tfChat.setIputType(TField.INPUT_TYPE_ANY);
		...
		if (idAction == 2001)
		{
			...
			Service.gI().useItem(0, (!flag) ? ((sbyte)1) : ((sbyte)0), (sbyte)((!flag) ? b : selected), -1);
		...
		if (idAction == 2003)
		{
			...
			Service.gI().useItem(1, (!flag2) ? ((sbyte)1) : ((sbyte)0), (sbyte)((!flag2) ? b2 : selected), -1);
		}
		if (idAction == 2004)
		{
			GameCanvas.endDlg();
		...
		if (idAction == 3002)
		{
			GameCanvas.endDlg();
			Item item12 = (Item)p;
			Service.gI().saleItem(0, (selected > 6) ? ((sbyte)1) : ((sbyte)0), (short)((selected >= 10) ? (selected - 10) : selected));
		}
		if (idAction == 3003)
		{
			GameCanvas.endDlg();
		...
		if (idAction == 5001)
		{
			...
			Service.gI().clanRemote(member4.ID, 0);
		}
		if (idAction == 5002)
		{
			Member member5 = (Member)p;
			Service.gI().clanRemote(member5.ID, 1);
		}
		if (idAction == 5003)
		{
			Member member6 = (Member)p;
			Service.gI().clanRemote(member6.ID, 2);
		}
		if (idAction == 5004)
		{
			Member member7 = (Member)p;
			Service.gI().clanRemote(member7.ID, -1);
		}
		if (idAction == 9000)
		{
		...
		if (idAction == 10000)
		{
			...
			Service.gI().enemy(1, infoItem6.charInfo.charID);
			GameCanvas.panel.hideNow();
		}
		if (idAction == 10001)
		{
			InfoItem infoItem7 = (InfoItem)p;
			Service.gI().enemy(2, infoItem7.charInfo.charID);
			InfoDlg.showWait();
		}
		if (idAction == 10021)
		{
