...
public class Mob : IMapObject
{
	...
	public FunctionCharEffect.CharEffectTime mobEffectTime = new FunctionCharEffect.CharEffectTime();
	...
	{
		if (!FunctionGraphic.enableHideMob)
		{
			int size = TileMap.size;
	...
	public virtual void update()
	{
		...
		FunctionTrainMob.UpdateCountDieMob(this);
		GetFrame();
		if (blindEff)
		{
		...
		if (isFreez)
		{
			mobEffectTime.isTDHS = true;
			if (isDie)
			{
				seconds = 0;
				mobEffectTime.isTDHS = false;
				mobEffectTime.timeTDHS = 0;
			}
			else
			{
				mobEffectTime.lastTimeTDHS = mSystem.currentTimeMillis();
				mobEffectTime.timeTDHS = seconds + 1;
			}
			if (GameCanvas.gameTick % 5 == 0)
			{
				ServerEffect.addServerEffect(113, x, y, 1);
	...
	{
		if (FunctionGraphic.enableFreezeMob)
		{
			return;
		}
	...
	{
		if (FunctionGraphic.enableHideMob || isHide)
		{
			return;
		}
		StringHandle.paint(mFont.tahoma_7_green, g, "[" + getTemplate().mobTemplateId + "] " + getTemplate().name, x, y - h - 15, mFont.CENTER, mFont.tahoma_7, "border", mGraphics.zoomLevel);
		if (isMafuba)
		{
			if (!changBody)
			{
	...
	{
		FunctionTrainMob.MobStartDie(this);
		hp = 0;
		injureThenDie = true;
		hp = 0;
		status = 1;
	...
	{
		if (!FunctionGraphic.enableFreezeMob && isGetFr && isTypeNewMod() && arrMobTemplate[templateId].data != null)
		{
