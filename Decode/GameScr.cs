...
public class GameScr : mScreen, IChatable
{
	...
	public static void loadBg()
	{
		...
		imgLbtn = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btnl") : GameCanvas.loadImage("/mainImage/myTexture2dbtnl.png"));
		imgLbtnFocus = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btnlf") : GameCanvas.loadImage("/mainImage/myTexture2dbtnlf.png"));
		imgLbtn2 = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btnl2") : GameCanvas.loadImage("/mainImage/myTexture2dbtnl2.png"));
		imgLbtnFocus2 = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("btnlf2") : GameCanvas.loadImage("/mainImage/myTexture2dbtnlf2.png"));
		imgPanel = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("panel") : GameCanvas.loadImage("/mainImage/myTexture2dpanel.png"));
		imgPanel2 = GameCanvas.loadImage("/mainImage/panel2.png");
		imgHP = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("hp") : GameCanvas.loadImage("/mainImage/myTexture2dHP.png"));
		...
		imgMP = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("mp") : GameCanvas.loadImage("/mainImage/myTexture2dMP.png"));
		imgSkill = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("skill1") : GameCanvas.loadImage("/mainImage/myTexture2dskill.png"));
		imgSkill2 = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("skill2") : GameCanvas.loadImage("/mainImage/myTexture2dskill2.png"));
		imgMenu = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("menu") : GameCanvas.loadImage("/mainImage/myTexture2dmenu.png"));
		...
		imgChatPC = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("chat") : GameCanvas.loadImage("/pc/chat.png"));
		imgChatsPC2 = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("chat2") : GameCanvas.loadImage("/pc/chat2.png"));
		if (GameCanvas.isTouch)
		{
			...
			imgChat = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("chatPC") : GameCanvas.loadImage("/mainImage/myTexture2dchat.png"));
			imgChat2 = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("chatPC2") : GameCanvas.loadImage("/mainImage/myTexture2dchat2.png"));
			imgFocus2 = GameCanvas.loadImage("/mainImage/myTexture2dfocus2.png");
			imgHP1 = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("pea0") : GameCanvas.loadImage("/mainImage/myTexture2dPea0.png"));
			imgHP2 = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("pea1") : GameCanvas.loadImage("/mainImage/myTexture2dPea1.png"));
			...
			imgHP3 = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("pea0") : GameCanvas.loadImage("/mainImage/myTexture2dPea2.png"));
			imgHP4 = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("pea1") : GameCanvas.loadImage("/mainImage/myTexture2dPea3.png"));
		...
		imgNut = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("nut") : GameCanvas.loadImage("/mainImage/myTexture2dnut.png"));
		imgNutF = (FunctionGraphic.enableSpecialUI ? FunctionGraphic.createUI("nutF") : GameCanvas.loadImage("/mainImage/myTexture2dnutF.png"));
		MobCapcha.init();
	...
	{
		sbyte[] array = Rms.loadRMS("skill" + Char.myCharz().charID);
		if (array != null && array.Length != 0)
		{
			for (int i = 0; i < array.Length; i++)
			{
				onScreenSkill[i] = findSkillId(array[i]);
			}
		}
	}

	public static Skill findSkillId(int id)
	{
		for (int i = 0; i < Char.myCharz().nClass.skillTemplates.Length; i++)
		{
			...
			if (id == skillTemplate.id)
			{
				return Char.myCharz().getSkill(skillTemplate);
			}
		}
		return null;
	}
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
						...
						else if (!FunctionMain.HotKeys(GameCanvas.keyAsciiPress) && GameCanvas.keyAsciiPress == 114)
						{
	...
	{
		if (FunctionTrainMob.IsTanSat || FunctionTrainMob.IsBlockTDLT)
		{
			return;
		}
		if (timeSkill > 0)
	...
	{
		if (Char.myCharz().isCreateDark || Char.myCharz().taskMaint.taskId <= 1)
		{
			return;
		}
		Char.myCharz().myskill = skill;
	...
	{
		FunctionMain.updateKeyTouchController();
		if (isNotPaintTouchControl())
		{
			return;
		}
	...
	public override void update()
	{
		...
		try
		{
			...
			{
				Effect2 effect = (Effect2)Effect2.vEffect2.elementAt(n);
				effect.update();
			}
	...
	public override void paint(mGraphics g)
	{
		...
		for (int num3 = 0; num3 < vCharInMap.size(); num3++)
		{
			...
			if (char3 != null && (!GameCanvas.panel.isShow || !GameCanvas.panel.isTypeShop()) && !FunctionGraphic.enableHideChar)
			{
				char3.paint(g);
			}
		}
		...
		for (int num4 = 0; num4 < vCharInMap.size(); num4++)
		{
			...
			if (char4 != null && (!GameCanvas.panel.isShow || !GameCanvas.panel.isTypeShop()) && char4.skillPaint != null && char4.skillInfoPaint() != null && char4.indexSkill < char4.skillInfoPaint().Length && !FunctionGraphic.enableHideChar)
			{
		...
		{
			Npc npc2 = (Npc)vNpc.elementAt(num6);
			npc2.paintName(g);
		}
		EffecMn.paintLayer3(g);
		if (!FunctionGraphic.enableHideNpc)
		{
		...
		if (!FunctionGraphic.enableHideChar)
		{
		...
		if (flag && !isRongThanXuatHien)
		{
			...
			if (TileMap.tileID != 28)
			{
				...
				{
					int num14 = 100 + num13 * (mGraphics.getImageWidth(TileMap.imgLight) + 50) - cmx / 2;
					int num15 = -20;
					int imageWidth = mGraphics.getImageWidth(TileMap.imgLight);
					if (num14 + imageWidth >= cmx && num14 <= cmx + GameCanvas.w && num15 + mGraphics.getImageHeight(TileMap.imgLight) >= cmy && num15 <= cmy + GameCanvas.h)
					{
						g.drawImage(TileMap.imgLight, 100 + num13 * (mGraphics.getImageWidth(TileMap.imgLight) + 50) - cmx / 2, num15, 0);
					}
				}
			}
		}
		if (!FunctionGraphic.enableHideChar || !FunctionGraphic.enableHideMob)
		{
			mSystem.paintFlyText(g);
		}
		...
		if (!isPaintOther)
		{
			...
			if ((TileMap.mapID == 128 || TileMap.mapID == 127) && mabuPercent != 0)
			{
				...
				g.setColor(16711680);
				g.setClip(num16 - 25, num17 - 110, mabuPercent, 4);
			...
			for (int num18 = 0; num18 < vCharInMap.size(); num18++)
			{
				Char char6 = null;
				try
				{
					char6 = (Char)vCharInMap.elementAt(num18);
				}
				catch (Exception)
				{
				}
			...
			resetTranslate(g);
			int num19 = 100 + ((Char.vItemTime.size() != 0) ? (textTime.size() * 12) : 0);
			if (Char.myCharz().clan != null)
			{
				int num20 = 0;
				int num21 = 0;
				int num22 = (GameCanvas.h - 100 - 60) / 12;
				for (int num23 = 0; num23 < vCharInMap.size(); num23++)
				{
					Char char7 = (Char)vCharInMap.elementAt(num23);
					...
					else if (char7.isOutX() && char7.cx > Char.myCharz().cx && num21 <= num22)
					{
			...
			for (int num25 = 0; num25 < Char.vItemTime.size(); num25++)
			{
				((ItemTime)Char.vItemTime.elementAt(num25)).paint(g, cmdMenu.x + 32 + num25 * 24, 55);
			}
			for (int num26 = 0; num26 < textTime.size(); num26++)
			{
				((ItemTime)textTime.elementAt(num26)).paintText(g, cmdMenu.x + ((Char.vItemTime.size() == 0) ? 25 : 5), ((Char.vItemTime.size() == 0) ? 45 : 90) + num26 * 12);
			}
			paintXoSo(g);
		...
		paintPhuBanBar(g, num28 + GameCanvas.w / 2, 0, num29);
	...
	private void paintImageBar(mGraphics g, bool isLeft, Char c)
	{
		...
		StringHandle.paint(mFont.tahoma_7_white, g, NinjaUtil.getMoneys(Char.myCharz().cHP), 85, 4, 0, mFont.tahoma_7_red, "border", mGraphics.zoomLevel);
		StringHandle.paint(mFont.tahoma_7_white, g, NinjaUtil.getMoneys(Char.myCharz().cMP), 85, 19, 0, mFont.tahoma_7_blue, "border", mGraphics.zoomLevel);
		if (FunctionChar.enableAutoTeleport)
		{
			StringHandle.paint(mFont.tahoma_7b_white, g, "Sức mạnh: " + NinjaUtil.getMoneys(Char.myCharz().cPower), 85, 40, 0, mFont.tahoma_7b_dark, "border", mGraphics.zoomLevel);
			StringHandle.paint(mFont.tahoma_7b_white, g, "Bấm X để hủy giảm sức mạnh", 85, 50, 0, mFont.tahoma_7b_dark, "border", mGraphics.zoomLevel);
		}
	}
	...
	private void paintInfoBar(mGraphics g)
	{
		...
		{
			mFont tahoma_7b_green = mFont.tahoma_7b_green2;
			tahoma_7b_green = (FunctionGraphic.enableSpecialUI ? mFont.tahoma_7b_white : mFont.tahoma_7b_green2);
			...
			else if (Char.myCharz().mobFocus != null)
			{
				...
				{
					tahoma_7b_green.drawString(g, Char.myCharz().mobFocus.getTemplate().name, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				}
				if (Char.myCharz().mobFocus.templateId != 0)
				{
					tahoma_7b_green.drawString(g, NinjaUtil.getMoneys(Char.myCharz().mobFocus.hp) + string.Empty, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				}
			}
			else if (Char.myCharz().npcFocus != null)
			{
				tahoma_7b_green.drawString(g, Char.myCharz().npcFocus.template.name, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				if (Char.myCharz().npcFocus.template.npcTemplateId == 4)
				{
					tahoma_7b_green.drawString(g, gI().magicTree.currPeas + "/" + gI().magicTree.maxPeas, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				}
			}
			else if (Char.myCharz().charFocus != null)
			{
				tahoma_7b_green.drawString(g, Char.myCharz().charFocus.cName, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				tahoma_7b_green.drawString(g, NinjaUtil.getMoneys(Char.myCharz().charFocus.cHP) + string.Empty, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
			}
			else
			{
				tahoma_7b_green.drawString(g, Char.myCharz().cName, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
				tahoma_7b_green.drawString(g, NinjaUtil.getMoneys(Char.myCharz().cPower) + string.Empty, imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
			}
		}
		g.translate(-g.getTranslateX(), -g.getTranslateY());
		if (isVS() && secondVS > 0)
	...
	private void paintSelectedSkill(mGraphics g)
	{
		...
		{
			if (FunctionGraphic.enableHideGameUI || GameCanvas.currentDialog != null || ChatPopup.currChatPopup != null || GameCanvas.menu.showMenu || isPaintPopup() || GameCanvas.panel.isShow || Char.myCharz().taskMaint.taskId == 0 || ChatTextField.gI().isShow || GameCanvas.currentScreen == MoneyCharge.instance)
			{
				return;
			}
			long num = mSystem.currentTimeMillis();
			...
			if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num4; i++)
				{
					if (Main.isPC)
					{
						...
						if (keySkill[i] != null)
						{
							StringHandle.paint(mFont.tahoma_7_white, g, array2[i], xSkill + xS[i] + 14, yS[i] + num5 + 1, mFont.CENTER, mFont.tahoma_7b_dark, "border", mGraphics.zoomLevel);
						}
					}
					...
					if (Char.myCharz().charEffectTime.hasShield && skill.template.id == 19)
					{
						StringHandle.paint(mFont.tahoma_7_white, g, Char.myCharz().charEffectTime.timeShield + "s", xSkill + xS[i] + 14, yS[i] + 18, mFont.CENTER, mFont.tahoma_7b_dark, "border", mGraphics.zoomLevel);
					}
					if (Char.myCharz().charEffectTime.hasMonkey && skill.template.id == 13)
					{
						StringHandle.paint(mFont.tahoma_7_white, g, Char.myCharz().charEffectTime.timeMonkey + "s", xSkill + xS[i] + 14, yS[i] + 18, mFont.CENTER, mFont.tahoma_7b_dark, "border", mGraphics.zoomLevel);
					}
					if (Char.myCharz().charEffectTime.hasMobMe && skill.template.id == 12)
					{
						StringHandle.paint(mFont.tahoma_7_white, g, Char.myCharz().charEffectTime.timeMobMe + "s", xSkill + xS[i] + 14, yS[i] + 18, mFont.CENTER, mFont.tahoma_7b_dark, "border", mGraphics.zoomLevel);
					}
					long num6 = skill.coolDown - mSystem.currentTimeMillis() + skill.lastTimeUseThisSkill;
					StringHandle.paint(mFont.tahoma_7b_white, g, (num6 < 0) ? string.Empty : ((num6 > 1000) ? (num6 / 1000).ToString() : TimeSpan.FromMilliseconds(num6).TotalSeconds.ToString()), xSkill + xS[i] + 14, yS[i] + 7, mFont.CENTER, mFont.tahoma_7, "border", mGraphics.zoomLevel);
				}
			}
			paintGamePad(g);
		}
	...
	public void chatVip(string chatVip)
	{
		...
		ListBossInformation.AddBoss(chatVip);
	...
	{
		if (!FunctionGraphic.enableHideServerNofitication && vChatVip.size() != 0 && isPaintChatVip)
		{
