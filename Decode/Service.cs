...
public class Service
{
	...
	public void clanMessage(int type, string text, int clanID)
	{
		...
		try
		{
			...
			if (type == 0)
			{
				if (StringHandle.Unikey.enableUnikey)
				{
					message.writer().writeUTF(StringHandle.Unikey.ConvertText(text));
				}
				else
				{
	...
	{
		if (FunctionMap.enableLockChangeZone)
		{
			return;
		}
