...
public class TileMap
{
	...
	{
		if (FunctionGraphic.enableHideTileMap)
		{
			return;
		}
		if (!FunctionGraphic.enableOptimizingTileMap)
		{
		...
		try
		{
			...
			for (int n = GameScr.gssx; n < GameScr.gssxe; n++)
			{
				...
				{
					if (maps[num5 * tmw + n] != 0 && ((!tileTypeAt(n * 24, (num5 + 1) * 24, 2) && !tileTypeAt(n * 24, (num5 + 2) * 24, 2) && !tileTypeAt(n * 24, num5 * 24, 2)) || tileTypeAt(n * 24, num5 * 24, 2)))
					{
						if (n > 0)
						{
	...
	{
		if (FunctionGraphic.enableHideBgItem || GameCanvas.lowGraphic)
		{
			return;
		}
		int num = 0;
