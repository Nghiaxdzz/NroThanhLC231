...
public class Main : MonoBehaviour
{
	...
	{
		Time.timeScale = 2f;
		FunctionLogin.LoadDefaultSetting();
		if (started)
		{
			return;
		}
		...
		if (isPC)
		{
			...
			int width = int.Parse(arguments["size"].Split('x')[0]);
			int height = int.Parse(arguments["size"].Split('x')[1]);
			Screen.SetResolution(width, height, fullscreen: false);
			FunctionClient.currentUsername = arguments["acc"];
		}
	}
	...
	private void OnGUI()
	{
		if (count >= 10)
		{
			...
			if (Event.current.type.Equals(EventType.Repaint) && paintCount <= updateCount)
			{
				...
				paintCount++;
				g.reset();
				FunctionLogin.paintWaitLogin(g);
			}
		}
	}
	...
	{
		if (FunctionGraphic.enableOptimizingCPU)
		{
			int millisecondsTimeout = (FunctionTrainMob.IsTanSat ? 15 : 40);
