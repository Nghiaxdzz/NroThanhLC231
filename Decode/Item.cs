...
public class Item
{
	...
	public void PaintColor(mGraphics g, int p1, int p2, int p3)
	{
		if (FunctionGraphic.enableHideItem || p3 < 4 || FunctionGraphic.enableOptimizingCPU)
		{
			return;
		}
		if (p3 > 7)
		...
		if (p3 >= 4)
		{
			g.setColor(array2[p3]);
			g.fillRect(p1 - 18, p2 - 12, 34, 23);
		}
		if (p3 >= 4 || p3 == 2)
		{
			for (int i = 0; i < 4; i++)
			{
				...
				{
					int num = p1 - 17 + paintcolor1(GameCanvas.gameTick + array3[i] - j * 4);
					int num2 = p2 - 12 + paintcolor2(GameCanvas.gameTick + array4[i] - j * 4);
					g.setColor(colorBorder[array[p3 - 1]][j]);
		...
		if (p3 == 1)
		{
			for (int k = 0; k < 2; k++)
			{
				...
				{
					int num3 = p1 - 17 + paintcolor1(GameCanvas.gameTick + array3[k] - l * 4);
					int num4 = p2 - 12 + paintcolor2(GameCanvas.gameTick + array4[k] - l * 4);
					g.setColor(colorBorder[array[p3 - 1]][l]);
		...
		for (int m = 0; m < 2; m++)
		{
			...
			{
				int num5 = p1 - 17 + paintcolor1(GameCanvas.gameTick + array3[m] - n * 4);
				int num6 = p2 - 12 + paintcolor2(GameCanvas.gameTick + array4[m] - n * 4);
				g.setColor(colorBorder[0][n]);
				g.fillRect(num5 - size[n] / 2, num6 - size[n] / 2, size[n], size[n]);
			}
		}
		for (int num7 = 2; num7 < 4; num7++)
		{
			for (int num8 = 0; num8 < size.Length; num8++)
			{
				int num9 = p1 - 17 + paintcolor1(GameCanvas.gameTick + array3[num7] - num8 * 4);
				int num10 = p2 - 12 + paintcolor2(GameCanvas.gameTick + array4[num7] - num8 * 4);
				g.setColor(colorBorder[1][num8]);
				g.fillRect(num9 - size[num8] / 2, num10 - size[num8] / 2, size[num8], size[num8]);
			}
		}
	}

	public int paintcolor1(int p0)
	{
		int num = 32;
		int num2 = p0 % 128;
		if (0 <= num2 && num2 < num)
